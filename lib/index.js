...
const resolvers$1 = {
	...
	Mutation: {
		postImg: (_, { img }) =>
			generalRequest(`${URL$1}/img2pdf`, 'POST', img),
		postPdf: (_, { pdf }) =>
			generalRequest(`${URL$1}/pdf2img`, 'POST', pdf),
		postPdfRange: (_, { firstPage, lastPage, pdf }) =>
			generalRequest(`${URL$1}/pdf2img/${firstPage}-${lastPage}`, 'POST', pdf),
	}
};
...
const resolvers$5 = {
	...
	Mutation: {
		postLogin: (_, { logininfo }) => 
			generalRequest(`${URL$5}/login?email=${logininfo.email}&password=${logininfo.password}`, 'POST',)
			,
